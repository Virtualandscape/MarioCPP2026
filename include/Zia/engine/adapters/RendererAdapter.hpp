...
namespace zia::engine::adapters {
    // Simple adapter that exposes the concrete Zia::Renderer via the IRenderer interface.
    class RendererAdapter : public IRenderer {
    public:
        explicit RendererAdapter(std::shared_ptr<zia::Renderer> r) : _renderer(std::move(r)) {}
        ...
        [[nodiscard]] std::shared_ptr<zia::Renderer> underlying() const { return _renderer; }

    private:
        std::shared_ptr<zia::Renderer> _renderer;
    };
} // namespace Zia::engine::adapters
